...
if (isset($_POST['newupdob'])) {
    $newdob = $_POST['newupdob'];
    $newphoneno = $_POST['newupphoneno'];
    $newupfname = $_POST['newupfname'];
    $newuplname = $_POST['newuplname'];
    $newupemail = $_POST['newupemail'];
    /*echo $newdob . $newphoneno;
    echo "hellomajama";*/
    $sql46 = "UPDATE `users` SET `date_of_birth`='$newdob',`phone_no`='$newphoneno',`f_name`='$newupfname',`l_name`='$newuplname',`email_id`='$newupemail' WHERE `user_id` = '$user_id'";
    $rel46 = mysqli_query($conn, $sql46);
}
...
<body>
    ...
    <div class="side-2">
        ...
        <form action="account_page.php" method="post">
            <?php
            $sql45 = "SELECT * FROM `users` WHERE `user_id` = '$user_id'";
            $rel45 = mysqli_query($conn, $sql45);
            $row = mysqli_fetch_assoc($rel45); // Fetch once, no loop needed for single user
            if ($row) {
                $fname = $row['f_name'];
                $lname = $row['l_name'];
                $email = $row['email_id'];
            ...
            <div class="ad-m">
                <div class="form-grid">
                    <div class="in-f">
                        <label class="la-in-f">First Name</label>
                        <div class="in">
                            <input type="text" name="newupfname" placeholder="Enter First Name" value="<?php echo $fname; ?>">
                        </div>
                    </div>
                    
                    <div class="in-f">
                        <label class="la-in-f">Last Name</label>
                        <div class="in">
                            <input type="text" name="newuplname" placeholder="Enter Last Name" value="<?php echo $lname; ?>">
                        </div>
                    </div>

                    <div class="in-f">
                        <label class="la-in-f">Phone No.</label>
                        <div class="in">
                            <input type="number" name="newupphoneno" placeholder="Enter Phone Number" value="<?php echo $phone_no; ?>">
                        </div>
                    </div>

                    <div class="in-f">
                        <label class="la-in-f">Date of Birth</label>
                        <div class="in">
                            <input type="date" name="newupdob" value="<?php echo $dob; ?>">
                        </div>
                    </div>

                    <div class="in-f full-width">
                        <label class="la-in-f">Email ID</label>
                        <div class="in">
                            <input type="email" name="newupemail" placeholder="Enter Email Address" value="<?php echo $email; ?>">
                        </div>
                    </div>
                </div>

                <div class="form-actions">
                    <button class="btn" name="update_person" value="1">Save Changes</button>
                </div>
            </div>
        </form>
