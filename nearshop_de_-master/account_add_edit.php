...
if (isset($_POST['edit_address_id'])) {
    $edit_id = $_POST['edit_address_id'];
    $action_label = "Edit Address";
    $sql_fetch = "SELECT `user_address` FROM `user_addresses` WHERE `address_id` = '$edit_id' AND `user_id` = '$user_id'";
    $res = mysqli_query($conn, $sql_fetch);
    if ($row = mysqli_fetch_assoc($res)) {
        $address_val = $row['user_address'];
    }
}

// Handle Form Submission
if (isset($_POST['save_address'])) {
    $new_addr = $_POST['address_content'];
    $aid = $_POST['address_id_hidden'];

    if (!empty($aid)) {
        // Update
        $sql_upd = "UPDATE `user_addresses` SET `user_address` = '$new_addr' WHERE `address_id` = '$aid' AND `user_id` = '$user_id'";
        mysqli_query($conn, $sql_upd);
    } else {
        // Insert
        // Check if it's the first address, make default
        $check = mysqli_query($conn, "SELECT 1 FROM `user_addresses` WHERE `user_id` = '$user_id'");
        $is_def = (mysqli_num_rows($check) == 0) ? 1 : 0;
        
        $sql_ins = "INSERT INTO `user_addresses` (`user_id`, `user_address`, `is_default`) VALUES ('$user_id', '$new_addr', '$is_def')";
        mysqli_query($conn, $sql_ins);
    }
    
    // Redirect back
    echo '<script>window.location.replace("account_page_address.php");</script>';
    exit();
}
...
<body>
    ...
    <div class="side-2">
        ...
        <hr>
        <div class="ad-m" style="max-width: 600px;">
            <form action="account_add_edit.php" method="post">
                <input type="hidden" name="address_id_hidden" value="<?php echo $edit_id; ?>">
                
                <div class="in-f full-width">
                    <label class="la-in-f">Address Details</label>
                    <div class="in">
                        <textarea name="address_content" style="width: 100%; height: 120px; padding: 12px; border: 1px solid var(--border-color); border-radius: 8px; font-size: 1rem; font-family: inherit; resize: vertical;" placeholder="Enter your full address here..." required><?php echo $address_val; ?></textarea>
                    </div>
                </div>

                <div class="form-actions" style="margin-top: 20px;">
                    <button class="btn" name="save_address" value="1">Save Address</button>
                    <a href="account_page_address.php" style="margin-left: 20px; display: inline-flex; align-items: center; color: var(--text-light); text-decoration: none; font-weight: 500;">Cancel</a>
                </div>
            </form>
        </div>
    </div>
